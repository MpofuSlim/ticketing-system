 ...
 *
 * <p><b>Response body logging is OFF by default.</b> Flip
 * {@code logging.access.bodies=true} only in local dev when debugging a
 * specific call — never in prod (bodies often contain PII + log volume cost).
 */
@Configuration
@Slf4j
    ...
    public FilterRegistrationBean<OncePerRequestFilter> httpAccessLogFilterRegistration() {
        ...
        reg.setFilter(new OncePerRequestFilter() {
            ...
            protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
                ...
                // Debug-only branch — wraps response so the body can be logged.
