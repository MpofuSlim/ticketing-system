 ...
 * text only (no HTML flag), so email copy matches the SMS/WhatsApp standard.
 * Used for system-user onboarding / credential / reset notifications. Any
 * rejection / connectivity failure surfaces as
 * {@link NotificationDeliveryException} so callers keep best-effort / fallback
 * semantics. Never logs the subject or message body (carries credentials).
 */
@Slf4j
@Component
